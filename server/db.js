...
// DB lives in ../data relative to /server
...
// Messages (audit trail for outbound comms)
db.exec(`
CREATE TABLE IF NOT EXISTS messages (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  loadId TEXT,
  toRole TEXT,        -- 'driver' | 'dispatcher' (can extend)
  toPhone TEXT,
  body TEXT,
  createdAt TEXT,
  fromRole TEXT,      -- optional
  fromName TEXT       -- optional
);
`);

// Partners (shipper/receiver organizations)
db.exec(`
CREATE TABLE IF NOT EXISTS partners (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  type TEXT,           -- 'shipper' | 'receiver' | (we also reuse for 'merchant'/'dispatcher' elsewhere)
  name TEXT,
  phone TEXT,
  email TEXT,
  notes TEXT,
...
// Recipient mapping per location (legacy portal UI used this)
db.exec(`
...
try {
  ...
  console.error('Migration (loads) failed:', e.message);
}
...
// Normalize to E.164 (US default if 10 digits). Accepts already + prefixed.
...
export function upsertLoad(load) {
  ...
  const insert = db.prepare(`INSERT INTO loads
...
export function updateLoadPartial(id, patch) {
  ...
  // Refresh lane if origin/destination changed
...
export function logMessage(m) {
  const now = nowISO();
  const stmt = db.prepare(`INSERT INTO messages (loadId, toRole, toPhone, body, createdAt, fromRole, fromName)
...
export function listMessagesByLoad(loadId) {
  return db
    .prepare('SELECT * FROM messages WHERE loadId = ? ORDER BY datetime(createdAt) ASC')
    .all(loadId);
}

/* =========================
   Partners, Locations, Recipients (portal)
========================= */
...
export function createPartner(p) {
  ...
  const r = db
    .prepare(
      `INSERT INTO partners (type, name, phone, email, notes, createdAt, updatedAt)
...
export function updatePartner(id, patch) {
  ...
  db.prepare(
    `UPDATE partners SET type=@type, name=@name, phone=@phone, email=@email, notes=@notes, updatedAt=@updatedAt WHERE id=@id`
  ).run({ ...row, id });
...
export function listLocations(partnerId) {
  return db
    .prepare('SELECT * FROM locations WHERE partnerId = ? ORDER BY name')
    .all(partnerId);
}
...
export function createLocation(loc) {
  ...
  const r = db
    .prepare(
      `INSERT INTO locations (partnerId, name, address, city, state, zip, lat, lng, createdAt, updatedAt)
...
export function updateLocation(id, patch) {
  ...
  db.prepare(
    `UPDATE locations SET name=@name, address=@address, city=@city, state=@state, zip=@zip, lat=@lat, lng=@lng, updatedAt=@updatedAt WHERE id=@id`
  ).run({ ...row, id });
...
// Per-location recipient mapping (legacy)
...
export function addLocationRecipient(locationId, partnerId, channel = 'sms') {
  const t = nowISO();
  db.prepare(
    'INSERT INTO location_recipients (locationId, partnerId, channel, createdAt) VALUES (?,?,?,?)'
  ).run(locationId, partnerId, channel, t);
...
/* =========================
   Recipients (newer, simple table used by hook)
========================= */

// This is the more flexible recipients list we read for location notifications
export function listRecipients(locationId) {
  // If you later add a dedicated 'recipients' table, map to it here.
  // For now, reuse location_recipients + partners as a resolved list.
  // (Kept to satisfy server import without breaking earlier data.)
  const rows = listLocationRecipients(locationId);
  // Map to a normalized shape with notify flags defaulting on for SMS
  return rows.map(r => ({
    id: r.id,
    locationId: r.locationId,
    partnerId: r.partnerId,
    role: r.partnerType || 'other',
    name: r.partnerName || '',
    phone: normPhone(r.partnerPhone),
    email: r.partnerEmail || '',
    notifySMS: r.channel === 'sms',
    notifyEmail: r.channel === 'email'
  }));
}

/* =========================
   Merchants / Dispatchers directories
========================= */
...
export function createMerchant(m) {
  ...
  const r = db
    .prepare(
      `INSERT INTO merchants (name, phone, email, notes, createdAt, updatedAt)
       VALUES (@name, @phone, @email, @notes, @createdAt, @updatedAt)`
    )
    .run(row);
  return db.prepare('SELECT * FROM merchants WHERE id = ?').get(r.lastInsertRowid);
}
...
export function updateMerchant(id, m) {
  ...
  const row = {
    ...cur,
    ...m,
    phone: normPhone(m.phone ?? cur.phone),
    updatedAt: nowISO()
  };
  db.prepare(
    `UPDATE merchants SET name=@name, phone=@phone, email=@email, notes=@notes, updatedAt=@updatedAt WHERE id=@id`
  ).run({ ...row, id });
...
export function createDispatcher(d) {
  ...
  const r = db
    .prepare(
      `INSERT INTO dispatchers (name, phone, email, notes, createdAt, updatedAt)
       VALUES (@name, @phone, @email, @notes, @createdAt, @updatedAt)`
    )
    .run(row);
  return db.prepare('SELECT * FROM dispatchers WHERE id = ?').get(r.lastInsertRowid);
}
...
export function updateDispatcher(id, d) {
  ...
  const row = {
    ...cur,
    ...d,
    phone: normPhone(d.phone ?? cur.phone),
    updatedAt: nowISO()
  };
  db.prepare(
    `UPDATE dispatchers SET name=@name, phone=@phone, email=@email, notes=@notes, updatedAt=@updatedAt WHERE id=@id`
  ).run({ ...row, id });
